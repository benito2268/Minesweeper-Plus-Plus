...
namespace menu {
	...
	Toolbar::Toolbar(int numMines, engine::GameEngine* ref) {
		...
		if (!ref->isBossMode) {
			...
			//if in boss mode, set to 300 seconds (5 mins) and count down
			this->clock = std::make_shared<Display>(((engine::GameEngine::width * TILE_SIZE) - 80) + mine::gameState::boss_offset_x, (MENU_HEIGHT / 2) - 15, 3, 1, 0, DISPLAY, ref);
		}
