...
namespace engine {
	...
	void GameEngine::init() {
		...
		this->texture->loadFromMemory(mem, memSize, sf::IntRect(0, 0, 538, 238));
	...
	void GameEngine::update(float interval) {
		...
		if (started) {
			...
			//std::cout << (double)duration_cast<milliseconds>(high_resolution_clock::now() - this->delta).count() << std::endl;
				if (!this->isBossMode) {
			...
			if (this->ticks == 29 && !this->toolbar->clock->frozen) { //calibrated (tm)
			//std::cout << (double)duration_cast<milliseconds>(high_resolution_clock::now() - this->delta).count() << std::endl;
				if(this->ticks == 29 && !this->toolbar->clock->frozen) {
					this->delta = high_resolution_clock::now();
					this->toolbar->decClock();
					this->ticks = 0;
				}
	...
	void GameEngine::input() {
		...
		while (this->window->pollEvent(winEvent)) {
			...
			if (winEvent.type == sf::Event::Closed) {
				...
				if (winEvent.mouseButton.button == sf::Mouse::Left) {
					...
					if (winEvent.mouseButton.y > MENU_HEIGHT && mine::gameState::gstate == mine::PLAYING) {
						if (!this->isBossMode) {
							...
							mine::uncover(winEvent.mouseButton.x - mine::gameState::boss_offset_x, (winEvent.mouseButton.y - MENU_HEIGHT) - mine::gameState::boss_offset_y, false);
						}
					}
					else {
						//check which button it's over
						//toolbar does not work in boss mode
						if (!this->isBossMode) {
				...
				else if(winEvent.key.code == sf::Keyboard::X) {
					enterBossMode();
				}
				...
				if (setEvent.type == sf::Event::MouseButtonPressed) {
					...
					if (setEvent.mouseButton.button == sf::Mouse::Left) {
						...
						if (this->gset->curr == menu::NEW_GAME) {
							...
							if (this->gset->death->isMouseOver(setEvent.mouseButton.x, setEvent.mouseButton.y)) {
								this->gset->death->press();
							}
						}
	...
	void GameEngine::enterBossMode() {
		//check if the user actually wants to
		int msgBoxID = MessageBox(
			NULL,
			(LPCWSTR)L"Fair Warning! This mode can crash your computer. It should be harmless, but I make no promises :) Are you sure you want to continue?",
		...
		//resize the screen and shift everything
		float screen_dim_x = sf::VideoMode::getDesktopMode().width;
		float screen_dim_y = sf::VideoMode::getDesktopMode().height;

		boss::bossFuncs::b_init(screen_dim_x, screen_dim_y);

		mine::init(mine::CUSTOM, mine::DEATH, this, 99, 24, 24, screen_dim_x, screen_dim_y);
		resetToolbar();

		this->isBossMode = true;
		for (int i = 0; i < mine::gameState::rows; i++) {
			for (int k = 0; k < mine::gameState::cols; k++) {
				sf::Sprite* s = mine::gameState::gboard->getptr(i, k)->sprite.get();
				s->setColor(sf::Color(255, 255, 255, 128));
			}
		}

		float dragBarHeight = 70;
		float xOffset = 20;

		this->window = std::make_shared<sf::RenderWindow>(sf::VideoMode(screen_dim_x + xOffset, screen_dim_y + dragBarHeight), this->title.c_str());
		this->window->setVerticalSyncEnabled(false);
		this->window->setFramerateLimit(60);
	}
