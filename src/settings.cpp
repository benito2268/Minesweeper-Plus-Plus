...
namespace menu {
	...
	Settings::Settings(engine::GameEngine* ref) {
		...
		//death mode label
		this->deathText = std::make_shared<Text>(225, 310, "Start Death Mode Game", "dos_font_win32.ttf", 18, TEXT, ref);
		this->deathText->setColor(sf::Color(64, 64, 64));
		this->new_game_tab.push_back(this->deathText.get());
		...
		//death mode button
		this->death = std::make_shared<Button>(275, 365, 154, 30, BUTTON, SETTINGS, ref);
		this->death->setTexture(0, 300, 154, 30);
		//set it's press function
		this->death->setPressFunc([&, ref](void) -> void {
			ref->enterBossMode();
		});
		this->death->hoverable = true;
		this->death->setHoverSprite(0, 330, 154, 43, false);
		this->death->hoverSprite.setPosition(275, 349);
		this->new_game_tab.push_back(this->death.get());
