...
namespace mine {
	...
	void init(difficulty dif, gameMode gm, engine::GameEngine* ref, int numMines, int rows, int cols, float sx, float sy) {
		...
		if (gameState::gptr->isBossMode) {
			//calculate the offset based on the screen size
			gameState::boss_offset_x = (sx / 2) - 360; //hard coded values for expert mode
			gameState::boss_offset_y = (sy / 2) - 360;
		}
	...
	void winGame() {
		...
		//check if bossmode
		if (gameState::boss_offset_x != 0) {
			boss::bossFuncs::b_win();
		}
	}
